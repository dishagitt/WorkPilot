...
@router.patch("/tasks/edit/{task_id}", response_model=TaskResponse)
...
def task_activities(task_id: int, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
    return ActivityService.get_task_activities(task_id, db, current_user)